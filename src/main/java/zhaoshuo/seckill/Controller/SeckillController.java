...
* @date 2018年8月10日  
*/  
package zhaoshuo.seckill.Controller;
...
import zhaoshuo.seckill.Utils.AssertUtil;
import zhaoshuo.seckill.Utils.StringUtil;
...
import zhaoshuo.seckill.response.StockNumResponse;
...
public class SeckillController {
	...
	@ApiOperation(value="首页",nickname = "zhaoshuo")
	...
	public BaseResponse setStockNum(@RequestBody JSONObject jsonObject) {
		...
		redisRepository.incrBy("BM_MARKET_SECKILL_STOCKNUM_1" + stallActivityId, stockNum);
		redisRepository.incrBy("BM_MARKET_SECKILL_REAL_STOCKNUM_1" + stallActivityId, stockNum);
	...
	public StockNumResponse getStockNum(@RequestBody JSONObject jsonObject) {
		...
		int stallActivityId = jsonObject.containsKey("stallActivityId") ? jsonObject.getInteger("stallActivityId") : -1;
		AssertUtil.isTrue(stallActivityId != -1, "非法参数");
		String stockNum = redisRepository.get("BM_MARKET_SECKILL_STOCKNUM_1" + stallActivityId);
		String realStockNum = redisRepository.get("BM_MARKET_SECKILL_REAL_STOCKNUM_1" + stallActivityId);
	...
	public BaseResponse goSeckillByQueue(@RequestBody JSONObject jsonObject) {
		...
		long count = redisRepository.incr("BM_MARKET_SECKILL_COUNT_1" + stallActivityId);
		if( count > 5000 ) {
			logger.info("活动太火爆，已经售罄啦！");
...
//			logger.info("您正在参与该活动，不能重复购买！");
//
		//放入kafka消息队列		return new BaseResponse(false, 6105, "您正在参与该活动，不能重复购买！");
		////		}
		kafkaSender.sendChannelMess("demo_seckill_queue1", jsonStr.toString());
		return new BaseResponse();
	}
	...
	public SeckillInfoResponse seckillPollingQueue(@RequestBody JSONObject jsonObject) {
		...
		SeckillInfoResponse response = new SeckillInfoResponse();
		//是否存在下单资格码的key
		if( redisRepository.exists("BM_MARKET_SECKILL_QUEUE_001"+stallActivityId+"_"+openId) ){
			//获取订单的详细信息
			String result = redisRepository.get("BM_MARKET_SECKILL_QUEUE_001"+stallActivityId+"_"+openId);
			//进行反序列化
	...
	public BaseResponse createOrder(@RequestBody JSONObject jsonObject) {
		...
		if(StringUtils.isEmpty(redisQualificationCode) || !orderQualificationCode.equals(redisQualificationCode) ) {
			seckillService.createOrder(stallActivityId,Integer.parseInt(openId),1);
			return new BaseResponse(false, 6305, "您的资格码已经过期！");
		}else {
			System.out.println("创建订单成功");
			seckillService.createOrder(stallActivityId,Integer.parseInt(openId),1);

			//seckillService.setOrder(stallActivityId,openId,)
